...
def run_symbolic(program, path, code=None, state=None, ctx=None, inclusive=False):
    ...
    history_states = list()
    while state.pc in program:
        # history_states.append(copy.deepcopy(state))
        ...
        if ((inclusive and len(path) == 0)
            ...
            # remove block_info constraints
            block_infos = ['GASPRICE', 'BLOCKHASH', 'COINBASE', 'TIMESTAMP', 'NUMBER', 'DIFFICULTY', 'GASLIMIT']
            ...
            for cons in constraints:
                ...
                for info in block_infos:
                    if info in str(cons):
                        break
                    _idx += 1
                if _idx == len(block_infos):
                    new_constraints.append(cons)
                else:
                    logging.info(f"remove block-info related constraints: {repr(str(z3.simplify(cons)))}" )
            
            return SymbolicResult(xid, history_states, state, new_constraints, sha_constraints, target_op)
        ...
        # Arithmetic
            ...
            if op == 'STOP':
                ...
                state.success = True
                constraints.append(z3.Or(*(z3.ULE(calldatasize, access) for access in calldata_accesses)))
                return SymbolicResult(xid,history_states, state, constraints, sha_constraints, target_op)
            elif op == 'ADD':
                stk.append(stk.pop() + stk.pop())
            elif op == 'SUB':
                stk.append(stk.pop() - stk.pop())
                ...
                s0, s1 = stk.pop(), stk.pop()
                if concrete(s1):
                    result = 0 if s1 == 0 else s0 % s1
                else:
                    result = z3.If(s1 == 0, z3.BitVecVal(0, 256), z3.URem(s0, s1))
                
                print(result)
                if any([_pt in str(result) for _pt in ['TIMESTAMP', 'DIFFICULTY', 'BLOCKHASH']]):
                    random_v = z3.BitVec('RANDOM_%x_%d' % (instruction_count, xid), 256)
                    random_constraints[random_v] = result
                    stk.append(random_v)
                else:
                    stk.append(result)
        ...
        # SHA3 and environment info
            ...
            if op == 'SHA3':
                ...
                if concrete(s0):
                    ...
                    stk.append(ctx_or_symbolic('CODESIZE-XC', ctx, xid))
                    # print(s0)
        ...
        # Block info
            ...
            if op == 'BLOCKHASH':
                ...
                if not concrete(s0):
                    ...
                    stk.append(ctx_or_symbolic('BLOCKHASH[0]',ctx, xid))
                else:
                    # print(s0)
                    stk.append(ctx_or_symbolic('BLOCKHASH[%d]' % s0, ctx, xid))
                # print("blockhash:", s0)
            elif op == 'COINBASE':
        ...
        # VM state manipulations
            ...
            if op == 'POP':
                ...
                if z3.is_bv_value(v):
                    ...
                    stk.append(v)
                # storage.mapping.append((state.pc, s0, v))
            elif op == 'SSTORE':
                s0, s1 = stk.pop(), stk.pop()
                storage[s0] = s1
            elif op == 'JUMP':
        ...
        # Calls
            ...
            if op in ('CALL', 'CALLCODE'):
                ...
                # print("call--s2=", s2)find a path
                if op == 'CALL':
                    constraints.append(z3.UGE(state.balance, s2))
                    state.balance -= s2
                    # history_states.append(SymbolicResult(xid,history_states, state, constraints, sha_constraints, target_op))
            ...
            if concrete(s1) and s1 <= 8:
                ...
                # p = project()
                # p.search_path(): filter error signature func
                # exec, get retVal
                # stk.push valid data#caller_storage[]
                # if :
                #     print('evm----------', ctx['xcall'])
                if ctx['xcall'] and ctx['xcall'][-1][0] == state.pc:
                    xretMem = ctx['xcall'][-1][1]
                    for i in range(olen):
                        mem[ostart + i] = xretMem[i]
                else:
                    for i in range(olen):
